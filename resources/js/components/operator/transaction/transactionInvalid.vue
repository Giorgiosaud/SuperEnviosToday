    ...
    <div class="modal-card">
      <header class="modal-card-head">
        <h1 class="modal-card-title">
          Transacción numero {{ datosDeTransaccion.transaction_number }} ya existe
        </h1>
      </header>
      <section class="modal-card-body">
        ...
        <strong>{{ datosDeTransaccion.created_at|date }}
        </strong> a las <strong>{{ datosDeTransaccion.created_at|time }}</strong>
        y su status es <strong>{{ datosDeTransaccion.status }}</strong>
        <div class="title-2">
          Adjuntos:
        </div>
        <div
          v-for="attachment in datosDeTransaccion.attachments"
          :key="attachment.id">
          <img
            :src="attachment.path"
            :alt="attachment.name">
