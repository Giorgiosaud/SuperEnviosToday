        ...
        <div class="container">
            <div class="row">
                ...
                <div class="col-12 col-md-9">
                    <label for="query" class="label-base">Consulta</label> <input id="query" v-model="query"
                                                                                  class="input-base">
                </div>
            </div>
        </div>
            ...
            <div class="table-responsive">
                <table class="table">
                    ...
                    <tr v-for="user in users">
                        ...
                        <td>
                            <span class="cursor-pointer" @click="editUser(user)" data-toggle="modal"
                                  data-target="#user-modal">Editar</span>
                        </td>
                    </tr>
                    </tbody>
                </table>
            ...
            <div class="modal-dialog modal-xl" role="document">
                <div class="modal-content" v-if="selectedUser">
                    ...
                    <div class="modal-body">
                        ...
                        <v-select id="idn_type" index="name_id" name="idn_type" class="mb-3" :searchable="false"
                                  :clearable="false" :options='idnTypes' v-model="selectedUser.idn_type"
                        ></v-select>
                        <div class="form-group">
                            <label for="roles" class="label-base">Roles</label>
                            <select id="roles" class="mb-3 form-control" label="name" multiple v-model="selectedUser.roles">
                                <option v-for="role in roles" :value="role.name_id">{{role.name}}</option>
                            </select>
                        </div>
                        <label for="idn" class="label-base">Numero de Identificación</label>
                        <input id="idn" type="text" class="input-base" name="idn"
                               v-model="selectedUser.idn" required autofocus>
                        <label for="name" class="label-base">Nombre</label>
                        <input id="name" type="text" class="input-base"
                               name="name" v-model="selectedUser.name" required autofocus>
                        <label for="last_name" class="label-base">Apellido(s)</label>
                        <input id="last_name" type="text"
                               class="input-base" name="last_name"
                               v-model="selectedUser.last_name" required autofocus>
                        <label for="phone" class="label-base">Telefono</label>
                        <input id="phone" type="text"
                               class="input-base" name="phone" v-model="selectedUser.phone" required autofocus>
                        <label class="label-base" for="email">Email</label>
                        <input class="input-base" type="email" id="email"
                               name="email" v-model="selectedUser.email" required autofocus>
                        <label for="address" class="label-base">Dirección</label>

                        <textarea id="address" class="input-base"
                                  v-model="selectedUser.address" required autofocus name="address"></textarea>
                    </div>
...
<script>
    export default {
        ...
        methods: {
            ...
            gotoUsersPage(page) {
                window.axios.get('/api/users', {params: {page: page, q: this.query}}).then(response => {
                    this.setData(response.data);
                });
            },
            goPrev() {
